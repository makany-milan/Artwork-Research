    ...
    try:
        ...
        if not match.empty:
            ...
            if matchValue == 1:
                ...
                dfx.loc[row[0], 'NNvalidInput'] = None
                dfx.loc[row[0], 'NNvalue'] = None
                dfx.loc[row[0], 'NNgender'] = None
                return True
            elif matchValue == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--folder', required=True, action='store', dest='folder')
    parser.add_argument('--file', required=True, action='store', dest='file')
    ...
    df = pd.read_csv(r'D:\oxford\rawdata\GenderData\training_data.csv')
    # Shuffle the data
    ...
    model_loc = os.path.join(r'C:\Users\Milan\OneDrive\Desktop\Oxford-Art\gender_NN', 'model.tf')


    weights = os.path.exists(model_loc)
    if not weights:
    ...
    '''''
    dfx = pd.read_csv(os.path.join(fileloc, filename))
    # dfx= dfx.head(500)
    pbar = tqdm(total=len(dfx.index))
    ...
    if 'forename' in dfx.columns:
        ...
        pool = ThreadPool(50)
        r = pool.map(classify, dfx.iterrows())
        pool.close()
    ...
    export_filename = filename.split('.')[0] + '-done.csv'
    export_loc = os.path.join(fileloc, export_filename)

    dfx.to_csv(export_loc, mode='w', index=False, sep=';')
